...
import com.felix.face.BuildConfig
import kotlin.math.*
...
class MediaPipeLivenessEngine(private val context: Context) {
    ...
    companion object {
        /**
         * 设置日志开关
         * @param enabled true 开启日志，false 关闭日志（默认关闭）
         */
        @JvmStatic
        fun setLogEnabled(enabled: Boolean) {
            FaceLog.setLogEnabled(enabled)
        }
         ...
         */
        @JvmStatic
        fun getVersion(): String = BuildConfig.FACEAI_VERSION
    }

    private var faceMesh: FaceMesh? = null
    private var currentScene: Int? = null
    ...
    // 人脸检测状态（用于避免频繁通知）
    private var lastFaceDetectedState = false  // 上次是否检测到人脸

    // 动作检测状态
    private var blinkCount = 0
    private var blinkState =
        BlinkState.OPEN  // 眨眼状态：OPEN(睁眼) -> CLOSING(闭眼中) -> CLOSED(闭眼) -> OPENING(睁眼中)
    ...
    private var expectedAction: Int? = null
    ...
    fun init(callback: EngineCallback) {
        ...
        try {
            ...
            FaceLog.d("MediaPipe FaceMesh初始化完成")
            callback.onInit(FaceCode.CODE_SUCCESS)
        } catch (e: UnsatisfiedLinkError) {
            FaceLog.e("JNI库加载失败", e)
            callback.onInit(FaceCode.CODE_JNI_LOAD_FAILED)
        } catch (e: Exception) {
            FaceLog.e("初始化失败", e)
            // 如果后续需要区分相机初始化失败，可在相机初始化处单独回调 CODE_CAMERA_INIT_FAILED
            callback.onInit(FaceCode.CODE_ENGINE_ERROR)
        }
    }
    ...
    private fun processFaceMeshResult(result: FaceMeshResult) {
        ...
        if (result.multiFaceLandmarks().isEmpty()) {
            FaceLog.d("未检测到人脸")
            // 如果上次检测到人脸，现在未检测到，通知状态变化
            if (lastFaceDetectedState) {
                lastFaceDetectedState = false
            }
            callback?.onTip(scene, expectedAction, FaceCode.CODE_NO_FACE_DETECTED)
        ...
        FaceLog.d("检测到人脸，当前场景: $scene")
        
        // 如果上次未检测到人脸，现在检测到了，通知状态变化
        if (!lastFaceDetectedState) {
            lastFaceDetectedState = true
            callback?.onTip(scene, expectedAction, FaceCode.CODE_FACE_DETECTED)
        }
     ...
     * 更新进度
     */
    private fun updateProgress(scene: Int) {
    ...
    private fun checkFaceInFrame(
        ...
        if (now % 500 < 50) {  // 大约每500ms输出一次
            FaceLog.d(
                "检测条件详情: distance=$distanceFromCenter (阈值=${frameRadius * 0.95f}), isInFrame=$isInFrame, " +
        ...
        if (isInFrame && isSizeFit && areCornersInside && isWithinBounds && isFrontFace) {
            ...
            if (faceInFrameStartTime == 0L) {
                ...
                if (duration >= faceInFrameRequiredDuration) {
                    FaceLog.d("人脸识别成功：连续${duration}ms在框内")
                    // 只在 FACE_IN_FRAME 场景时回调，FACE_IN_FRAME_CAPTURE 场景不回调
                    if (scene == Scene.FACE_IN_FRAME) {
                        notifySceneCompleted(scene, null, FaceCode.CODE_SUCCESS)
                    }
                    faceInFrameStartTime = 0L
                    return true  // 返回true表示已入框
                } else {
                    // 提示保持（检测中）
                    callback?.onTip(scene, null, FaceCode.CODE_NOT_IN_FRAME)
                }
            }
        } else {
            // 不在框内或不是正面，重置计时
            if (faceInFrameStartTime != 0L) {
                FaceLog.d("重置计时：isInFrame=$isInFrame, isSizeFit=$isSizeFit, isFrontFace=$isFrontFace, distance=$distanceFromCenter, size=$faceSize, yaw=${headPose.yaw}, pitch=${headPose.pitch}")
                faceInFrameStartTime = 0L
            }
            when {
                !isInFrame -> callback?.onTip(scene, null, FaceCode.CODE_NOT_IN_FRAME)
                !isFrontFace -> callback?.onTip(scene, null, FaceCode.CODE_FACE_NOT_FRONT)
                faceSize > 0.8f || faceSize < 0.1f -> callback?.onTip(
                    scene,
                    null,
                    FaceCode.CODE_FACE_SIZE_INVALID
                )
            }
        }
     ...
     * @param landmarks 人脸关键点
     */
    private fun checkAction(scene: Int, landmarks: List<LandmarkProto.NormalizedLandmark>) {
        // 如果没有设置期望的动作，直接返回
        ...
        if (!isInFrame || !isSizeFit || !areCornersInside || !isWithinBounds) {
            // 不在框内，提示用户
            callback?.onTip(scene, expectedAction, FaceCode.CODE_NOT_IN_FRAME)
            return
        }
        ...
        when (expectedAction) {
                ...
                when (blinkState) {
                    BlinkState.OPEN -> {
                        ...
                        if (ear < 0.20f) {
                            ...
                            blinkCount = 1
                            FaceLog.d("眨眼检测: 开始闭眼，EAR=$ear, 状态: OPEN -> CLOSING")
                        }
                    }
                    ...
                    BlinkState.CLOSING -> {
                        ...
                        if (ear < 0.20f) {
                            ...
                            if (ear >= 0.25f) {
                                ...
                                blinkCount = 0
                                FaceLog.d("眨眼检测: 闭眼中断，EAR=$ear（>=0.25），状态: CLOSING -> OPEN")
                            } else {
                                // EAR在0.20-0.25之间时，保持CLOSING状态，允许波动
                                FaceLog.d("眨眼检测: 闭眼中波动，EAR=$ear（0.20-0.25之间），保持CLOSING状态，count=$blinkCount")
                            }
                        }
                    }
                    ...
                    BlinkState.CLOSED -> {
                        ...
                        if (ear >= 0.18f) {
                            ...
                            blinkCount = 1
                            FaceLog.d("眨眼检测: 开始睁眼，EAR=$ear, 状态: CLOSED -> OPENING")
                        } else {
                            // 每500ms输出一次闭眼状态的日志
                            if (now % 500 < 50) {
                                FaceLog.d("眨眼检测: 保持闭眼状态，EAR=$ear, 等待睁眼（EAR>=0.18）")
                            }
                        }
                    }
                    ...
                    BlinkState.OPENING -> {
                        ...
                        if (ear >= 0.18f) {
                            blinkCount++
                            FaceLog.d("眨眼检测: 睁眼中，EAR=$ear, count=$blinkCount")
                            // 折中方案：如果EAR较高（>=0.25），1帧即可；否则需要2帧
                            if (blinkCount >= 1 && ear >= 0.25f) {
                                // EAR较高，确认是睁眼，1帧即可（支持快眨眼）
                                FaceLog.d("眨眼动作完成（快速），EAR=$ear, 状态: OPENING -> OPEN")
                                blinkState = BlinkState.OPEN
                                blinkCount = 0
                                // 清除当前动作，防止重复检测
                                ...
                                expectedAction = null
                                completedAction?.let {
                                    notifySceneCompleted(
                                        scene,
                                        it,
                                        FaceCode.CODE_SUCCESS
                                    )
                                }
                            } else if (blinkCount >= 2) {
                                // EAR在0.18-0.25之间，需要2帧确认（防止误判）
                                FaceLog.d("眨眼动作完成（自定义检测），EAR=$ear, 状态: OPENING -> OPEN, 完整周期完成")
                                blinkState = BlinkState.OPEN
                                blinkCount = 0
                                // 清除当前动作，防止重复检测
                                val completedAction = expectedAction
                            ...
                            if (ear < 0.15f) {
                                ...
                                blinkState = BlinkState.CLOSED
                                blinkCount = 0
                                FaceLog.d("眨眼检测: 睁眼中断，EAR=$ear（<0.15），状态: OPENING -> CLOSED")
                            } else if (ear < 0.18f && blinkCount >= 1) {
                                // EAR在0.15-0.18之间，且已经检测到至少1帧睁眼，允许波动，不中断
                                FaceLog.d("眨眼检测: 睁眼中波动，EAR=$ear（0.15-0.18之间），已检测到${blinkCount}帧睁眼，保持OPENING状态")
                            } else {
                                // EAR在0.15-0.18之间，但count=0，可能是刚开始睁眼，允许波动
                                FaceLog.d("眨眼检测: 睁眼中波动，EAR=$ear（0.15-0.18之间），保持OPENING状态，count=$blinkCount")
                            }
                        }
                    }
                }
            ...
            ActionType.MOUTH_OPEN -> {
                ...
                if (mar > 0.38f) {
                    mouthOpenCount++
                    FaceLog.d("张嘴检测: MAR=$mar（>0.38，张嘴中）, count=$mouthOpenCount/2")
                    // 减少所需帧数：从3帧降到2帧（约0.07秒@30fps），让检测更灵敏
                    if (mouthOpenCount >= 2) {
                        FaceLog.d("张嘴动作完成（自定义检测），MAR=$mar, count=$mouthOpenCount, 状态: 完成")
                        // 清除当前动作，防止重复检测
                        val completedAction = expectedAction
                        expectedAction = null
                        mouthOpenCount = 0
                    ...
                    if (mar >= 0.35f && mouthOpenCount > 0) {
                        // 允许波动，不重置计数
                        FaceLog.d("张嘴检测: MAR=$mar（0.35-0.38之间，波动中），保持计数=$mouthOpenCount")
                    } else {
                        // 每500ms输出一次MAR值，帮助调试
                        if (mar < 0.35f) {
                            if (mouthOpenCount > 0) {
                                FaceLog.d("张嘴检测中断，MAR=$mar（<0.35）, count=$mouthOpenCount, 重置计数")
                            } else if (now % 500 < 50) {
                                // 每500ms输出一次当前MAR值（当计数为0时）
                                FaceLog.d("张嘴检测: MAR=$mar（<0.38，未张嘴）, count=0, 等待张嘴（MAR>0.38）")
                            }
                        }
                        mouthOpenCount = 0
                    }
            ...
            ActionType.SHAKE_HEAD -> {
                ...
                if (yawAbs > 90f) {
                    if (now % 500 < 50) {
                        FaceLog.d("摇头检测: yaw异常（yawAbs>90度），跳过自定义检测。yaw=$yaw")
                    }
                    return
                }
                ...
                when (headShakeState) {
                    HeadShakeState.CENTER -> {
                        ...
                        if (isFrontFace) {
                            ...
                            if (headShakeInitialYaw == 0f) {
                                ...
                                FaceLog.d("摇头检测: yaw=$yaw, yawAbs=$yawAbs, 状态: CENTER, 已记录初始yaw（正面）, initialYaw=$headShakeInitialYaw, 等待开始摆动")
                            }
                            ...
                            if (yaw < -10f) {
                                ...
                                if (headShakeStartedFromCenter) {
                                    ...
                                    headShakeCount = 1
                                    FaceLog.d("摇头检测: yaw=$yaw, yawAbs=$yawAbs, 状态: CENTER -> LEFT, 从正面开始摆动（初始yaw=$headShakeInitialYaw）")
                                }
                            } else if (yaw > 10f) {
                                // 摆到右侧（必须从正面开始）
                                if (headShakeStartedFromCenter) {
                                    ...
                                    headShakeMaxYawValue = yaw
                                    headShakeCount = 1
                                    FaceLog.d("摇头检测: yaw=$yaw, yawAbs=$yawAbs, 状态: CENTER -> RIGHT, 从正面开始摆动（初始yaw=$headShakeInitialYaw）")
                                }
                            } else {
                                // 每500ms输出一次当前yaw值
                                if (now % 500 < 50) {
                                    FaceLog.d("摇头检测: yaw=$yaw, yawAbs=$yawAbs（正面，-15到15度）, 状态: CENTER, initialYaw=$headShakeInitialYaw, 等待摆动（yaw>10度或yaw<-10度）")
                                }
                            }
                        } else {
                            // 不在正面，重置初始值
                            if (headShakeInitialYaw != 0f || headShakeStartedFromCenter) {
                                FaceLog.d("摇头检测: yaw=$yaw, yawAbs=$yawAbs（不在正面，>10度）, 状态: CENTER, 重置初始值，等待回到正面")
                                headShakeInitialYaw = 0f
                                headShakeStartedFromCenter = false
                            } else {
                                // 每500ms输出一次当前yaw值
                                if (now % 500 < 50) {
                                    FaceLog.d("摇头检测: yaw=$yaw, yawAbs=$yawAbs（不在正面，>10度）, 状态: CENTER, 等待回到正面（yaw在-10到10度之间）")
                                }
                            }
                        }
                    }
                    ...
                    HeadShakeState.LEFT -> {
                        ...
                        if (yaw < -10f) {
                            // 继续在左侧
                            headShakeCount++
                            FaceLog.d("摇头检测: yaw=$yaw, yawAbs=$yawAbs, 状态: LEFT, minYaw=$headShakeMinYaw, maxYaw=$headShakeMaxYawValue, initialYaw=$headShakeInitialYaw, count=$headShakeCount, 继续在左侧")
                        } else if (yaw > 10f) {
                            // 快速摆到右侧，完成一个完整的左右摆动
                            headShakeMaxYaw = maxOf(headShakeMaxYaw, yawAbs)
                            FaceLog.d("摇头动作完成（快速摇头检测），yaw=$yaw, yawAbs=$yawAbs, 状态: LEFT -> RIGHT, maxYaw=$headShakeMaxYaw, initialYaw=$headShakeInitialYaw, count=$headShakeCount, 完整周期完成")
                            val completedAction = expectedAction
                            expectedAction = null
                            completedAction?.let {
                                notifySceneCompleted(
                            ...
                            FaceLog.d("摇头动作完成（轻微回中），yaw=$yaw, headShakeMinYaw=$headShakeMinYaw, initialYaw=$headShakeInitialYaw, count=$headShakeCount")
                            val completedAction = expectedAction
                            expectedAction = null
                            completedAction?.let {
                                notifySceneCompleted(
                            ...
                            if (headShakeMinYaw < -10f && headShakeMaxYawValue > 10f && yawRange > 20f) {
                                FaceLog.d("摇头动作完成（缓慢摇头检测），yaw=$yaw, minYaw=$headShakeMinYaw, maxYaw=$headShakeMaxYawValue, range=$yawRange（>20度，跨越左右两侧）, initialYaw=$headShakeInitialYaw, count=$headShakeCount, 完整周期完成")
                                val completedAction = expectedAction
                                expectedAction = null
                                completedAction?.let {
                                    notifySceneCompleted(
                                ...
                                headShakeInitialYaw = 0f
                                headShakeStartedFromCenter = false
                                FaceLog.d("摇头检测中断，yaw=$yaw, yawAbs=$yawAbs, maxYaw=$headShakeMaxYaw（<20度，摆动幅度不够）, 状态: LEFT -> CENTER, 重置")
                            } else {
                                // 摆动幅度足够，允许回到中间（可能是过渡）
                                FaceLog.d("摇头检测: yaw=$yaw, yawAbs=$yawAbs, 状态: LEFT（过渡到中间）, maxYaw=$headShakeMaxYaw（>=20度，幅度足够）, initialYaw=$headShakeInitialYaw, count=$headShakeCount")
                            }
                        }
                    }
                    ...
                    HeadShakeState.RIGHT -> {
                        ...
                        if (yaw > 10f) {
                            // 继续在右侧
                            headShakeCount++
                            FaceLog.d("摇头检测: yaw=$yaw, yawAbs=$yawAbs, 状态: RIGHT, minYaw=$headShakeMinYaw, maxYaw=$headShakeMaxYawValue, initialYaw=$headShakeInitialYaw, count=$headShakeCount, 继续在右侧")
                        } else if (yaw < -10f) {
                            // 快速摆到左侧，完成一个完整的右左摆动
                            headShakeMaxYaw = maxOf(headShakeMaxYaw, yawAbs)
                            FaceLog.d("摇头动作完成（快速摇头检测），yaw=$yaw, yawAbs=$yawAbs, 状态: RIGHT -> LEFT, maxYaw=$headShakeMaxYaw, initialYaw=$headShakeInitialYaw, count=$headShakeCount, 完整周期完成")
                            val completedAction = expectedAction
                            expectedAction = null
                            completedAction?.let {
                                notifySceneCompleted(
                            ...
                            FaceLog.d("摇头动作完成（轻微回中），yaw=$yaw, headShakeMaxYawValue=$headShakeMaxYawValue, initialYaw=$headShakeInitialYaw, count=$headShakeCount")
                            val completedAction = expectedAction
                            expectedAction = null
                            completedAction?.let {
                                notifySceneCompleted(
                            ...
                            if (headShakeMinYaw < -10f && headShakeMaxYawValue > 10f && yawRange > 20f) {
                                ...
                                headShakeInitialYaw = 0f
                                headShakeStartedFromCenter = false
                                FaceLog.d("摇头检测中断，yaw=$yaw, yawAbs=$yawAbs, maxYaw=$headShakeMaxYaw（<20度，摆动幅度不够）, 状态: RIGHT -> CENTER, 重置")
                            } else {
                                // 摆动幅度足够，允许回到中间（可能是过渡）
                                FaceLog.d("摇头检测: yaw=$yaw, yawAbs=$yawAbs, 状态: RIGHT（过渡到中间）, maxYaw=$headShakeMaxYaw（>=20度，幅度足够）, initialYaw=$headShakeInitialYaw, count=$headShakeCount")
                            }
                        }
                    }
                }
            ...
            ActionType.NOD_HEAD -> {
                ...
                if (isNoddingByPitch || isNoddingByDistance) {
                    ...
                    if (headShakeCount >= 8) {  // 从12帧降低到8帧
                        FaceLog.d("点头动作完成（自定义检测），pitch=$pitch, distance=$noseChinDistance, base=$nodBaseDistance, increase=$distanceIncrease, count=$headShakeCount")
                        // 清除当前动作，防止重复检测
                        val completedAction = expectedAction
                        expectedAction = null
                        headShakeCount = 0
                    ...
                    if (headShakeCount > 0) {
                        FaceLog.d("点头检测中断，pitch=$pitch, distance=$noseChinDistance, base=$nodBaseDistance, increase=$distanceIncrease, count=$headShakeCount")
                    }
                    headShakeCount = 0
                    // 更新基准距离
     ...
     * @param landmarks 人脸关键点
     */
    private fun checkCapture(scene: Int, landmarks: List<LandmarkProto.NormalizedLandmark>) {
        // 根据场景决定是否检查框内和抓拍策略
        // FACE_IN_FRAME_CAPTURE场景：已经在checkFaceInFrame中检查过框内（连续2秒），这里立即抓拍
        // CAPTURE场景：检查框内，需要连续1秒在框内且正面才抓拍
        ...
        if (currentTime % 500 < 50) {  // 每500ms输出一次
            FaceLog.d("抓拍检测: scene=$scene, isImmediate=$isImmediateCapture, yaw=${headPose.yaw}, pitch=${headPose.pitch}, isFrontFace=$isFrontFace, readyTime=${if (captureReadyStartTime > 0) currentTime - captureReadyStartTime else 0}ms")
        }
        ...
        if (isFrontFace) {
            if (isImmediateCapture) {
                ...
                analysisBitmap?.let { bitmap ->
                    FaceLog.d("抓拍成功（立即抓拍），bitmap大小: ${bitmap.width}x${bitmap.height}, 旋转角度: $currentRotationDegrees")
                    // 如果图片需要旋转，先旋转再抓拍
                    val capturedBitmap = if (currentRotationDegrees != 0) {
                        rotateBitmap(bitmap, currentRotationDegrees)
                    } else {
                        bitmap
                    }
                    val config = capturedBitmap.config
                    notifyCapture(scene, capturedBitmap.copy(config, false))
                    notifySceneCompleted(scene, null, FaceCode.CODE_SUCCESS)
                } ?: run {
                    FaceLog.w("抓拍失败：analysisBitmap为null")
                    notifySceneCompleted(scene, null, FaceCode.CODE_CAPTURE_BITMAP_NULL)
                }
            } else {
                ...
                if (captureReadyStartTime == 0L) {
                    ...
                    if (duration >= captureReadyRequiredDuration) {
                        // 连续满足条件达到要求时间，执行抓拍
                        FaceLog.d("抓拍条件满足，连续${duration}ms在框内且正面，开始抓拍")
                        analysisBitmap?.let { bitmap ->
                            FaceLog.d("抓拍成功，bitmap大小: ${bitmap.width}x${bitmap.height}, 旋转角度: $currentRotationDegrees")
                            // 如果图片需要旋转，先旋转再抓拍
                            val capturedBitmap = if (currentRotationDegrees != 0) {
                                rotateBitmap(bitmap, currentRotationDegrees)
                            } else {
                            ...
                            val config = capturedBitmap.config ?: Bitmap.Config.ARGB_8888
            ...
            // 不满足条件，重置计时
            if (captureReadyStartTime != 0L) {
                FaceLog.d("抓拍准备中断，重置计时")
                captureReadyStartTime = 0L
            }
            callback?.onTip(scene, null, FaceCode.CODE_FACE_NOT_FRONT)
        }
    }
     ...
     */
    fun switchToScene(scene: Int, action: Int? = null) {
        // 验证：ACTION场景时，action不能为空
        if (scene == Scene.ACTION && action == null) {
            // 直接调用失败回调，不抛出异常
            notifySceneCompleted(scene, null, FaceCode.CODE_ACTION_TYPE_NULL)
            return
        }
        ...
        when (scene) {
            ...
            Scene.ACTION -> {
                ...
                FaceLog.d("切换到场景: $scene, 动作类型: $action")
            }
    ...
    fun processFrame(imageProxy: ImageProxy) {
        val image = imageProxy.image ?: run {
            FaceLog.d("processFrame: image为null")
            imageProxy.close()
            return
        }
        ...
        if (analysisBitmap == null || analysisBitmap?.width != imageProxy.width || analysisBitmap?.height != imageProxy.height) {
            ...
            FaceLog.d("创建新的analysisBitmap: ${imageProxy.width}x${imageProxy.height}")
        }
        ...
        analysisBitmap?.let { bitmap ->
            ...
            // 3. 保存当前旋转角度，用于后续抓拍时旋转图片
            currentRotationDegrees = imageProxy.imageInfo.rotationDegrees

            // 4. 处理旋转
            val rotatedBitmap = if (currentRotationDegrees != 0) {
                rotateBitmap(bitmap, currentRotationDegrees)
            } else {
                bitmap
            }
            ...
            if (faceMesh != null) {
                ...
                FaceLog.w("processFrame: faceMesh为null，无法处理")
            }

            // 如果进行了旋转，rotatedBitmap 是新创建的，需要适时回收（但在持续分析中通常依赖 GC）
            // analysisBitmap 是复用的，不需要手动 recycle
        } ?: run {
            FaceLog.w("processFrame: analysisBitmap为null")
        }
    ...
    private fun checkTimeout() {
        ...
        if (elapsed > sceneTimeout) {
            ...
            notifySceneCompleted(scene, action, FaceCode.CODE_TIMEOUT)
        }
    }
     ...
     * 通知场景完成（先清空场景状态，再回调）
     */
    private fun notifySceneCompleted(scene: Int, action: Int?, code: Int) {
        // 先清空场景状态
        clearSceneState()
        // 然后通知结果
        callback?.onCompleted(scene, action, code)
    }

    /**
     * 通知抓拍（先清空场景状态，再回调）
     */
    private fun notifyCapture(scene: Int, bitmap: Bitmap) {
        // 然后通知抓拍
        callback?.onCapture(scene, bitmap)
    }
